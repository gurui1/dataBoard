...
function CRUD (options) {
  ...
  const methods = {
    createTime () {
      const oldDate = moment().subtract(0, 'days').format('YYYY-MM-DD 00:00:00')
      const today = moment().subtract(0, 'days').format('YYYY-MM-DD 23:59:59')
    ...
    refresh () {
      ...
      return new Promise((resolve, reject) => {
        crud.loading = true
        // 请求数据
        if (crud.url.indexOf('http://10.161.48.85') === 0 ||
          crud.url.indexOf('http://10.236.36.168') === 0 ||
          // crud.url.indexOf('http://192.168.1.122') === 0 ||
          crud.url.indexOf('http://10.168.48.85') === 0) {
          const TIMESTAMP = moment().subtract(0, 'days').format('YYYY-MM-DD HH:mm:ss')
          crud.query.token = md5('APP_IDappealTIMESTAMP' + TIMESTAMP + ' 100B2732427')
          crud.query.timeStamp = TIMESTAMP + ' 100'
          initDataPost(crud.url, crud.getQueryParams(1)).then(res => {
            ...
            if (data.count !== undefined) {
              crud.page.total = Number(data.count)
            }
            // if (data.dataInfo !== undefined) {
            //   crud.data = data.dataInfo
            // }
            if (data.dataInfo) {
              if (JSON.parse(data.dataInfo) !== undefined) {
                crud.data = JSON.parse(data.dataInfo)
        ...
        } else if (crud.url.indexOf('http://10.177.36.41:8991') === 0) {
          initData(crud.url, crud.getQueryParams(2)).then(res => {
            const table = crud.getTable()
            const data = res.body
            if (table && table.lazy) { // 懒加载子节点数据，清掉已加载的数据
              table.store.states.treeData = {}
          ...
          initData(crud.url, crud.getQueryParams(1)).then(res => {
            const table = crud.getTable()
            const data = res.data
            if (table && table.lazy) { // 懒加载子节点数据，清掉已加载的数据
              table.store.states.treeData = {}
            ...
            if (data.content !== undefined) {
              ...
              if (typeof data === 'object') {
                ...
                const onlyObjects = keys.every(key => typeof data[key] === 'object')
                const hasSameLength = keys.length === Object.values(data).filter(value => typeof value === 'object').length
                if (onlyObjects && hasSameLength) {
                  crud.data = data
                } else {
                  crud.data = [data]
                }
    ...
    doAdd () {
      ...
      if (crud.url.indexOf('http://10.161.48.85') === 0 ||
        ...
        crud.url.indexOf('http://10.168.48.85') === 0) {
        const TIMESTAMP = moment().subtract(0, 'days').format('YYYY-MM-DD HH:mm:ss')
        crud.form.token = md5('APP_IDappealTIMESTAMP' + TIMESTAMP + ' 100B2732427')
        crud.form.timeStamp = TIMESTAMP + ' 100'
      }
    ...
    doDelete (data) {
      ...
      return crud.crudMethod.del(ids, data.type).then(() => {
        if (delAll) {
          crud.delAllLoading = false
        } else dataStatus.delete = CRUD.STATUS.PREPARED
        crud.dleChangePage(1)
        if (crud.title !== '信安工作通讯录') {
     ...
     * 通用导出
     */
    doExport (url, fileType) {
      let urlApi = ''

      if (crud.page.total > '500000') {
        Notification.error({
      ...
      if (typeof url === 'object' || url === undefined) {
        ...
        urlApi = process.env.VUE_APP_UASE_API + url
      }

      crud.downloadLoading = true
      let params = {}
      if (crud.url.indexOf('http://10.177.36.41:8991') === 0) {
        params = crud.getExportQueryParams()
      } else {
        params = crud.getQueryParams(1)
      }
      download(urlApi + '/export', params).then(result => {
        if (result !== undefined) {
          downloadFile(result, crud.title + '数据', fileType || 'xlsx')
          // 页面导出记录
          const params = {
            operType: '4',
            pageName: crud.title,
          ...
          initLogRecord(params).then(res => { })
        } else {
          // 导出公安关停号码数据的限制提示
          if (crud.title === '公安关停号码') {
            Notification.error({
    ...
    /**
     * 获取项目类型
     * @param {Number} type 项目type，1：信安2.0/行业反诈平台;  2：防诈骗互联网感知系统
    */
    ...
    getQueryParams (type) {
      ...
      if (type === 1) {
        ...
        return {
          startPosition: crud.page.page - 1,
          maxResult: crud.page.size,
          ...crud.query,
          ...crud.params
        }
      }
     ...
     * @param {Array} retain 保留字段，不进行重置
     */
    resetQuery (toQuery = true, retain = []) {
      const defaultQuery = JSON.parse(JSON.stringify(crud.defaultQuery))
      const query = crud.query
      Object.keys(query).forEach(key => {
        if (retain.length !== 0) {
          if (!this.arrayFindHandler(retain, key)) {
